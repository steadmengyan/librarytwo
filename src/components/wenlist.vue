<template>
    <div id="wenlist">
        ...
        <div class="list">
            ...
            <div class="item" @click="wendetail(key.id)"  v-for='key in arr'>
                <div class="bigDiv">
                    <span class="yd">【{{key.status}}】</span>
                    <span>{{key.question}}</span>
                    <div class="smallDiv">
                        <span class="txt">{{key.ques_time}}</span>
         ...
         <div class="button">
            <div class="bot">
                你 <span style="color:#f58823;text-decoration:underline">&nbsp;{{tiaomu.count}}条 &nbsp;</span>问题等待回答,请耐心等待。
            </div>
       </div>
 </div>   
    </div>
        ...
        var than = this;
         if(this.tokenId){
             this.$axios.defaults.headers.common["token"] =this.tokenId;
              this.$axios.post("http://xcx.xywol.com/index.php/api/version/MyQuestion?page=1")
              .then(function(res){
                 console.log("成功的回调");
               console.log(res.data.data);
               than.arr=res.data.data.result
               than.tiaomu=res.data.data
               })
               .catch(err=>{
                   console.log("失败的回调");
